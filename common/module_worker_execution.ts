...
export class ModuleWorkerExecution {
    ...
    static async create(scriptPath: string, bindings: Record<string, Binding>, callbacks: WorkerExecutionCallbacks): Promise<ModuleWorkerExecution> {
        const { globalCachesProvider, webSocketPairProvider, onModuleWorkerInfo, kvNamespaceProvider, doNamespaceProvider, r2BucketProvider, analyticsEngineProvider } = callbacks;
        ...
        applyWorkerEnv(moduleWorkerEnv, bindings, kvNamespaceProvider, doNamespaceProvider, r2BucketProvider, analyticsEngineProvider);
