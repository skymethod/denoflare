...
export async function getObject(opts: { bucket: string, key: string, origin: string, region: string, ifMatch?: string, ifNoneMatch?: string, ifModifiedSince?: string, ifUnmodifiedSince?: string }, context: AwsCallContext): Promise<Response> {
    const { bucket, key, origin, region, ifMatch, ifNoneMatch, ifModifiedSince, ifUnmodifiedSince } = opts;
