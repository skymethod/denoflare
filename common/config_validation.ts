...
function checkBinding(name: string, binding: any) {
    ...
    const { value, secret, kvNamespace, doNamespace, wasmModule, bucketName } = binding;
    const definedCount = [ value, secret, kvNamespace, doNamespace, wasmModule, bucketName ].filter(v => v !== undefined).length;
    if (definedCount === 1) {
