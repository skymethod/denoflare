...
export async function packResponse(response: Response, bodies: Bodies, webSocketPacker: WebSocketPacker, overrideContentType?: string): Promise<PackedResponse> {
    ...
    if (DenoflareResponse.is(response)) {
        ...
        if (typeof response.bodyInit === 'string') {
            ...
            return { status, statusText, headers, bodyId: undefined, bodyText, bodyBytes: undefined, bodyNull: false, webSocketId, url, redirected };
        } else if (response.bodyInit instanceof Uint8Array) {
            const bodyBytes = response.bodyInit;
            return { status, statusText, headers, bodyId: undefined, bodyText: undefined, bodyBytes, bodyNull: false, webSocketId, url, redirected };
            ...
            return { status, statusText, headers, bodyId, bodyText: undefined, bodyBytes: undefined, bodyNull: false, webSocketId, url, redirected };
            ...
            return { status, statusText, headers, bodyId: undefined, bodyText: undefined, bodyBytes: undefined, bodyNull: true, webSocketId, url, redirected };
        } else {
...
export function unpackResponse(packed: PackedResponse, bodyResolver: BodyResolver, webSocketResolver: WebSocketResolver): Response | DenoflareResponse {
    ...
    const rt = new _Response(body, { status, statusText, headers });
