import { resolve, fromFileUrl, toFileUrl } from 'https://deno.land/std@0.200.0/path/mod.ts'; // intended to be self-contained, don't use shared deps
 ...
 */
export async function importBinary(importMetaUrl: string, moduleSpecifier: string): Promise<ArrayBuffer> {
    if (moduleSpecifier.startsWith('https://')) {
        return await importBinaryFromHttps(moduleSpecifier);
    }

    if (importMetaUrl.startsWith('file://')) {
        return await (await fetch(appendQueryHint(toFileUrl(resolve(resolve(fromFileUrl(importMetaUrl), '..'), moduleSpecifier))).toString())).arrayBuffer();
        ...
        return await importBinaryFromHttps(binaryUrl);
    } else {
...
async function importBinaryFromHttps(url: string): Promise<ArrayBuffer> {
