import { deleteBucketEncryption as deleteBucketEncryptionR2, R2 } from '../common/r2/r2.ts';
...
export async function deleteBucketEncryption(args: (string | number)[], options: Record<string, unknown>) {
    if (DELETE_BUCKET_ENCRYPTION_COMMAND.dumpHelp(args, options)) return;

    const { verbose, bucket } = DELETE_BUCKET_ENCRYPTION_COMMAND.parse(args, options);
