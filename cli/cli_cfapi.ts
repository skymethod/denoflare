...
function cfapiCommand() {
    ...
    add(apiCommand('get-queue-metrics', 'Get metrics for a specific queue').arg('queueNameOrId', 'string', 'Queue name (or id)'), async (accountId, apiToken, opts) => {
        const queueId = await findQueueId({ accountId, apiToken, opts });
        const result = await getQueueMetrics({ accountId, apiToken, queueId });
        console.log({ ...result, oldest_message_timestamp: new Date(result.oldest_message_timestamp_ms).toISOString() });
    });
    ...
    add(apiCommand('send-queue-message', 'Send queue message').arg('queueNameOrId', 'string', 'Queue name (or id)')
            ...
            .option('delaySeconds', 'integer', 'The number of seconds to wait for attempting to deliver this message to consumers')
            , async (accountId, apiToken, opts) => {
        const queueId = await findQueueId({ accountId, apiToken, opts });
        const { json, text, delaySeconds: delay_seconds } = opts;

        const message: QueueMessagePayload | undefined = typeof json === 'string' ? { content_type: 'json', body: JSON.parse(json), delay_seconds }
            : typeof text === 'string' ? { content_type: 'text', body: text, delay_seconds }
            : undefined;
        if (message === undefined) throw new Error(`Provide the message using one of the 'json' or 'text' options`);

        await sendQueueMessage({ accountId, apiToken, queueId, message });
    });

    add(apiCommand('send-queue-message-batch', 'Send queue message batch').arg('queueNameOrId', 'string', 'Queue name (or id)')
            .option('json', 'strings', 'A JSON message')
            .option('text', 'strings', 'A text message')
            .option('delaySeconds', 'integer', 'The number of seconds to wait for attempting to deliver this batch to consumers')
            , async (accountId, apiToken, opts) => {
        const queueId = await findQueueId({ accountId, apiToken, opts });
        const { json = [], text = [], delaySeconds: delay_seconds } = opts;

        const batch: QueueMessageBatchPayload = {
            delay_seconds,
            messages: [
                ...json.map(v => ({ content_type: 'json' as const, body: JSON.parse(v) })),
                ...text.map(v => ({ content_type: 'text' as const, body: v })),
            ],
        };

        await sendQueueMessageBatch({ accountId, apiToken, queueId, batch });
    });
...
function wfp(rt: CliCommand<unknown>) {
    ...
    addWfp(wfpCommand('get-script-tags', 'List tags from a script uploaded to a dispatch namespace')
            .arg('dispatchNamespace', 'string', 'The name of the dispatch namespace')
            .arg('scriptName', 'string', 'The name of the script')
        , async (accountId, apiToken, { dispatchNamespace, scriptName }) => {
        const value = await getScriptTags({ accountId, apiToken, dispatchNamespace, scriptName });
        console.log(value);
    });

    addWfp(wfpCommand('put-script-tags', 'Put tags for a script uploaded to a dispatch namespace')
            .arg('dispatchNamespace', 'string', 'The name of the dispatch namespace')
            .arg('scriptName', 'string', 'The name of the script')
            .option('tag', 'strings', 'String tag')
        , async (accountId, apiToken, { dispatchNamespace, scriptName, tag: tags = [] }) => {
        const value = await putScriptTags({ accountId, apiToken, dispatchNamespace, scriptName, tags });
        console.log(value);
    });
    
    addWfp(wfpCommand('delete-script-tag', 'Delete a tag from a script uploaded to a dispatch namespace')
            .arg('dispatchNamespace', 'string', 'The name of the dispatch namespace')
            .arg('scriptName', 'string', 'The name of the script')
            .arg('tag', 'string', 'The string tag')
        , async (accountId, apiToken, { dispatchNamespace, scriptName, tag }) => {
        const value = await deleteScriptTag({ accountId, apiToken, dispatchNamespace, scriptName, tag });
        console.log(value);
    });

    addWfp(wfpCommand('list-scripts', 'List scripts in a dispatch namespace, with optional tag filter')
            .arg('dispatchNamespace', 'string', 'The name of the dispatch namespace')
            .option('tags', 'string', 'A comma-separated yes/no tag filter (e.g. my-tag:yes,another-tag:no)')
        , async (accountId, apiToken, { dispatchNamespace, tags }) => {
        const value = await listScriptsInDispatchNamespace({ accountId, apiToken, dispatchNamespace, tags });
        console.log(value);
    });

    addWfp(wfpCommand('delete-scripts', 'Delete scripts in a dispatch namespace meeting a tag filter')
            .arg('dispatchNamespace', 'string', 'The name of the dispatch namespace')
            .option('tags', 'required-string', 'A comma-separated yes/no tag filter (e.g. my-tag:yes,another-tag:no)')
        , async (accountId, apiToken, { dispatchNamespace, tags }) => {
        const value = await deleteScriptsInDispatchNamespace({ accountId, apiToken, dispatchNamespace, tags });
        console.log(value);
    });
